...
def find_all_mask(platform=None):
    if platform is None:
        platform = platform.system()
    ipstr = '([0-9]{1,3}\.){3}[0-9]{1,3}'
    maskstr = '0x([0-9a-f]{8})'
...
def find_all_broad(platform=None):
    if platform is None:
        platform = platform.system()
    ipstr = '([0-9]{1,3}\.){3}[0-9]{1,3}'
    if platform == "Darwin" or platform == "Linux":
...
def create_virtual_network(interface, addr):
    if os.geteuid():
        args = [sys.executable] + sys.argv
        os.execlp('su', 'su', '-c', ' '.join(args))

    cmd = "/sbin/ifconfig " + interface + addr + " up"
    os.popen(cmd)